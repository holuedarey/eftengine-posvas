...
class FrscNotifier {
    ...
    constructor(notificationData) {
        ...
        this.isLive = process.env.frsc_live == 'true' ? true : false;

        this.userName = this.isLive ? "GT9Lo02b67" : "GTB0987";
        this.apiKey = this.isLive ? "597FDNOVTLYOEFTDP5P0" : "S4OK57TM5YEFFWONDTD8";
    }
        ...
        try {
            ...
            if (registeredNotifications) {
                ...
                if (notificationServices) {
                    ...
                    this.notificationService = notificationServices.find(c => c.notificationClass == "manual-frsc");
    ...
    async sendNotification() {
        ...
        if (!this.notificationData.customerRef) {
            console.error(`customer data not found to send notification; FRSC Transaction TID :${this.notificationData.terminalId}, RRN :${this.notificationData.rrn}, at ${new Date().toString()}`);
            ...
            .then((response) => {
                ...
                try {
                    ...
                    Journal.updateOne({
                        ...
                        if (err)
                            console.error(`error updating FRSC notification result on journal at ${new Date().toString()} RRN : ${this.notificationData.rrn}`);
                        else
                            console.log(`FRSC notification result updated successfully at ${new Date().toString()} RRN : ${this.notificationData.rrn}`);
                    });
